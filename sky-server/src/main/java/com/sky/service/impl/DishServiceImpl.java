...
//    private RedisCacheUtil redisCacheUtil;
     ...
     */
    @Override
    @Cacheable(cacheNames = RedisConstant.DISH_CATEGORY,
            key = "#categoryId",
            unless = "#result == null || #result.isEmpty()")
    public List<DishVO> getDishByCategoryUser(Long categoryId) {
//        // 1. 构造redis中的key，规则为 dish_category_{categoryId}
//        String key = RedisConstant.DISH_CATEGORY_KEY + "dish_category_" + categoryId;
//
//        // 2. 先从redis当中查询分类数据
...
//        redisTemplate.opsForValue().set(key, dishVOS, 1, TimeUnit.HOURS);
    ...
    @CacheEvict(cacheNames = RedisConstant.DISH_CATEGORY, key = "#dishDTO.categoryId")
...
//        // 清除对应菜品类别的redis缓存
//        String key = RedisConstant.DISH_CATEGORY_KEY + "dish_category_" + dish.getCategoryId();
...
//        log.info("清除以下键的Redis缓存: {}", key);
     ...
     */
    @Override
    @CacheEvict(cacheNames = RedisConstant.DISH_CATEGORY, allEntries = true)
    public Integer updateDishStatus(Integer status, Long id) {
        // 清空redis缓存
//        log.info("清除菜品相关的所有Redis缓存");
...
//        String key = RedisConstant.DISH_CATEGORY_KEY + "*";
